  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        <aside
          className={cn(
            "sidebar-scroll flex-shrink-0 border-l border-border overflow-x-hidden overflow-y-auto scroll-smooth flex flex-col transition-all duration-300",
            narrativeCollapsed
              ? "w-12 bg-surface/40 backdrop-blur-sm"
              : narrativeExpanded
                ? "absolute right-0 top-0 bottom-0 z-50 w-[min(860px,100vw)] shadow-2xl border-l bg-surface"
                : "w-96 bg-surface/40 backdrop-blur-sm",
          )}
        >
          <NarrativePanel
            state={state}
